...
def create_ckpt_dir(config):
    ckpt_root = config.ckpt_root  # Load from config
    now = datetime.datetime.today()
    ckpt_dir = os.path.join(ckpt_root, "{0:%m%d_%H%M_%S}".format(now))
    os.makedirs(ckpt_dir, exist_ok=True) # Use makedirs instead of mkdir
    os.makedirs(os.path.join(ckpt_dir, "val_vis"), exist_ok=True) # Use makedirs instead of mkdir
    os.makedirs(os.path.join(ckpt_dir, "models"), exist_ok=True) # Use makedirs instead of mkdir
    return ckpt_dir
